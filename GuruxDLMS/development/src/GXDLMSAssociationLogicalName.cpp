...
int CGXDLMSAssociationLogicalName::Invoke(CGXDLMSSettings& settings, CGXDLMSValueEventArg& e)
{
    ...
    if (e.GetIndex() == 1)
    {
		...
		if (settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH_ECDSA)
		{
			...
			if ((ret = verify_HLS_authentication(&session_id, &auth_params, buffer, buffer_size, is_originator)) != 0)
			{
			...
			if ((ret = create_HLS_authentication(&session_id, &auth_params, buffer, &buffer_size, is_originator)) != 0)
				return ret;
